...
旁路口（Backup）是一个**可自动切换方向的接口**，支持以下场景：


**并网**

- 接入微逆：P ≤ -30W，持续5s → 微逆运行模式
  <img src={require("./img/grid_microinverter.png").default} width="480"/>

- 接入负载：P ≥ 30W，持续20s → EPS 运行模式
...
- 接入微逆：
  - 固件要求：EMS ≥ 1.01.05 支持旁路口连接微逆
  - P ≤ -30W，持续5s → 微逆运行模式
  - AC断开后，旁路口自动关闭，防止损坏微逆或逆变器
  - 如需供电：需重启设备切换为 EPS 运行模式


- 接入负载：
  - P ≥ 30W，持续20s → EPS 运行模式
  - AC断开后，旁路口自动切换到微储 EPS 供电
